...
irs_positions = np.vstack((x, y + irs_loc[1], z + irs_loc[2])). # combine into an array of IRS element positions
...
d_bi = np.linalg.norm(irs_loc - bs_loc) # BS-IRS distance 

d_iu  = np.zeros(n_ue) # vector storage for IRS-UE distance 

for k in range(n_ue):
    d_iu_k = np.linalg.norm(ue_loc[k] - irs_loc) # IRS-UE eulcidean distance calculation
    d_iu[k] = d_iu_k

pl_bi = 30 + 22 * np.log10(d_bi) # BS-IRS path loss
pl_iu = 30 + 22 * np.log10(d_iu) # IRS-UE path loss
